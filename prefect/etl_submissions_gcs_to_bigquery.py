...
def extract_from_gcs() -> pd.DataFrame:
    ...
    for blob in gcs_bucket.list_blobs("submissions"):
        print(blob.name)
...
def get_processed_submissions():
    with BigQueryWarehouse.load("de-zoomcamp-project-bq") as dw:
        operation = '''
            SELECT file_name
            FROM subreddit_posts.processed_submissions
        '''
...
def get_gcs_file_names():
    ...
    return {blob.name for blob in gcs_bucket.list_blobs("submissions")}
...
def extract_from_gcs(file_name):
    ...
    with BytesIO() as buf:
        ...
        print(file)
    submission = create_submission_dict(json.loads(file))
...
def log_progress_to_bq(bucket, file_name, df):
    print(f"Logging {file_name} to subreddit_posts.processed_submissions.")
    gcp_credentials_block = GcpCredentials.load("de-zoomcamp-project-creds")
    df = pd.DataFrame({
        'bucket': bucket.name,
        'file_name': file_name,
...
def etl_gcs_to_bigquery():
    ...
    processed = get_processed_submissions()
